...
const wordcount = await inquirer.prompt([
    ...
    {
        message: chalk.blue.bold.italic("Enter your sentence to count the words", "\n"), // Message displayed on the CLI
        type: "input", // The type is input
        name: "sentence", //with the name sentence
        validate: (input) => {
            const checkvalidation = input.trim() === "" ? chalk.red.bold("Please Enter Valid Word") : true; // Check if the user enters nothing; return this message if empty, otherwise return true.
